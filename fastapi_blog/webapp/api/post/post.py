...
async def read_posts(session: AsyncSession = Depends(get_session)):
    ...
    await redis_startup.redis.set(cache_key, serialized_posts, ex=60)  # Кэширование на 60 секунд
...
@post_router.get('/user/{user_id}', response_model=List[PostRead], response_class=ORJSONResponse)
